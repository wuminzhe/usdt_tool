...
(function() {
  const methods = Object.getOwnPropertyNames(BitcoinClient.prototype).filter(function(p) {
...
class OmniClient extends BitcoinPromisesClient {
   ...
   * @return Promise<BalanceArray|Error> a promise for the balance info
   */
...
(function() {
  for (const method in OmniClientCommands) {
    (function(methodName) {
...
Omni.sendissuancecrowdsale = function(params, cb) {
  const address = params.fromaddress;
  ...
  const tokensperunit = params.tokensperunit;

  const deadline = params.deadline;

  const earlybonus = params.earlybonus;
  const issuerpercentage = params.issuerpercentage;
